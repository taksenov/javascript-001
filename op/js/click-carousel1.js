...
jQuery.fn.clickCarousel = function(options)
{
	...
	if(typeof container1.animating == 'undefined' ) 
	{
	...
	shiftOnly: true,
	left: jQuery("#carouselRight1"),
	right: jQuery("#carouselLeft1")
	},
	...
	if(settings.shiftOnly === false)
	{
		...
		{		
			...
			else //The index must be put in the context of what's being shifted
			{
				...
				//Make sure settings.shifting is the container1 of what will be shifted...not the click-trigger
				clicked = settings.shifting[index];
			}
			
			//If the container1 is not animated
			if(container1.animating === false)
			{			
				//The container1 is now animating.
				...
				{		
					...
					{
						//The container1 has finished animating
						container1.animating = false;
					});					
				});			
			}
		...
		/*********************************************************/
			...
			if(isset(settings.left))
			{
				...
				{				
					//If the container1 is NOT animating, allow the following code to execute
					if(container1.animating === false)
					{
